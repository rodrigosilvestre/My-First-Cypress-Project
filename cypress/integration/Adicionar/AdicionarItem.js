...
When('é requisitada a inserção das descrições {string} {string} {string} {string} {string} {string} {string} do item', (nome, preço, estoque, país, moeda, usado,dataDeModificação) => {
    ...
    cy.get('[id="ocDatagridCell-TestGrid-isUsed-0"]').click().should('contain','Yes').click() //Escolhendo se o elemento é do tipo usado ou não
...
When('verifico se os campos obrigatorios foram preenchidos', () => {
    cy.get('[class="oc-datagrid-tooltip error"]')
});
